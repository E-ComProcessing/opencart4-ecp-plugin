...
namespace Opencart\Extension\Ecomprocessing\System\Catalog;
...
use Opencart\Catalog\Model\Extension\Ecomprocessing\Payment\EcomprocessingCheckout as ModelEcomprocessingCheckout;
...
abstract class BaseController extends Controller
{
	...
	protected function buildUrl($controller, $action = ''): string {
		if (!empty($action)) {
			$action_separator = EcomprocessingHelper::CONTROLLER_ACTION_SEPARATOR;
	...
	protected function populateTreedsParams($controller, $product_info, $order_info): array {
		...
		$ecomprocessing_threeds_helper     = new ThreedsHelper();

		/**
		 * Get all customer's orders
		 * Default limit is 20, and they are sorted in descending order
		 */
		$customer_orders                 = $ecomprocessing_threeds_helper->getCustomerOrders(
			$controller->db,
			$this->getCustomerId($controller),
			(int)$controller->config->get('config_store_id'),
			(int)$controller->config->get('config_language_id'),
			ModelEcomprocessingCheckout::METHOD_CODE
		);

		$is_guest                        = !$controller->customer->isLogged();
		$has_physical_products           = $ecomprocessing_threeds_helper->hasPhysicalProduct($product_info);
		$threeds_challenge_indicator     = $controller->config->get('ecomprocessing_checkout_threeds_challenge_indicator');

		$threeds_purchase_category       = $ecomprocessing_threeds_helper->hasPhysicalProduct($product_info) ? Categories::GOODS : Categories::SERVICE;
		$threeds_delivery_timeframe      = ($has_physical_products) ? DeliveryTimeframes::ANOTHER_DAY : DeliveryTimeframes::ELECTRONICS;
		$threeds_shipping_indicator      = $ecomprocessing_threeds_helper->getShippingIndicator($has_physical_products, $order_info, $is_guest);
		$threeds_reorder_items_indicator = $ecomprocessing_threeds_helper->getReorderItemsIndicator(
			$model_account_order,
		...
		if (!$is_guest) {
			$threeds_registration_date                = $ecomprocessing_threeds_helper->findFirstCustomerOrderDate($customer_orders);
			$threeds_registration_indicator           = $ecomprocessing_threeds_helper->getRegistrationIndicator($threeds_registration_date);
			$threeds_creation_date                    = $ecomprocessing_threeds_helper->getCreationDate($model_account_customer, $order_info['customer_id']);

			$shipping_address_date_first_used         = $ecomprocessing_threeds_helper->findShippingAddressDateFirstUsed(
				$model_account_order,
				$order_info,
				$customer_orders
			);
			$threads_shipping_address_date_first_used = $shipping_address_date_first_used;
			$threeds_shipping_address_usage_indicator = $ecomprocessing_threeds_helper->getShippingAddressUsageIndicator($shipping_address_date_first_used);

			$orders_for_a_period                      = $ecomprocessing_threeds_helper->findNumberOfOrdersForaPeriod(
				$model_account_order,
				$customer_orders
			);
	...
	protected function buildActionUrls($module_name): array {
		return [
			'notification_url'   =>
				$this->buildUrl(
					'extension/ecomprocessing/payment/' . $module_name,
					'callback'
				),
			'return_success_url' =>
				$this->buildUrl(
